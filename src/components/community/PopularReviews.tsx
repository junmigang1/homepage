...
const popularReviews = [
  ...
  },
  {
    ...
    rating: 5,
    content: '감정을 느끼지 못하는 소년의 이야기인데, 오히려 더 깊은 감동을 받았어요. 우리가 당연하게 여기는 감정들이...',
  ...
  },
  {
    ...
    rating: 5,
    content: '빅브라더가 당신을 지켜보고 있다... 이 한 줄이 얼마나 무서운지 이제야 알겠어요. 현실과 너무 닮아있어서...',
  ...
  },
  {
    ...
    rating: 4,
    content: '이 책에서 배운 공부법을 실제로 적용해봤는데 정말 효과가 있었어요! 특히 25분 집중 + 5분 휴식 사이클이...',
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {popularReviews.map((review) => (
            <div key={review.id} className="space-y-3 p-4 rounded-lg border hover:bg-muted/30 transition-colors">
              {/* 순위 */}
              <div className="flex items-center gap-2">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-sm font-bold ${
                  review.rank === 1 ? 'bg-yellow-100 text-yellow-800' :
                  review.rank === 2 ? 'bg-gray-100 text-gray-800' :
                  review.rank === 3 ? 'bg-orange-100 text-orange-800' :
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {review.user.isVerified && (
                      <Badge variant="default" className="text-xs">인증</Badge>
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground">{review.user.school}</p>
                </div>
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">{review.book.author}</p>
                </div>
              </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {topUsers.map((user) => (
            <div key={user.rank} className="flex items-center gap-3 p-3 rounded-lg hover:bg-muted/30 transition-colors">
