...
// UI에서 사용하는 확장된 Book 타입
interface UIBook extends Book {
  ownerSchool?: string
  location?: string
...
export function BookBrowse() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const transformedBooks = data.map((book: Book) => ({
            ...
            rating:
              (book as any).reviews?.length > 0
                ? (book as any).reviews.reduce(
                    (sum: number, r: { rating?: number }) =>
                      sum + (r.rating || 0),
                    0
                  ) / (book as any).reviews.length
                : 4.5,
            reviewCount: Array.isArray(book.reviewCount)
              ? book.reviewCount.length
              : book.reviewCount || 0,
  ...
  const filteredBooks = books.filter(book => {
    ...
    const matchesCondition =
      selectedCondition === '전체' || book.condition === selectedCondition
    const matchesSchool =
      selectedSchool === '전체' || book.ownerSchool === selectedSchool

    return matchesSearch && matchesGenre && matchesCondition && matchesSchool
  })

  const handleExchangeRequest = (book: UIBook) => {
      ...
      >
        <DialogContent>
            ...
            <div className="space-y-4">
              <div className="flex gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm">
                    소유자:{' '}
                    {typeof selectedBook.owner === 'string'
                      ? selectedBook.owner
                      : selectedBook.owner?.name || '알 수 없음'}
                  </p>
                  <p className="text-sm">
                    교환방식: {selectedBook.exchangeMethod}
                  </p>
                </div>
              </div>
              <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button onClick={confirmExchange}>교환 요청하기</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
